...
export async function fetchDeepSeekMotivation(daysFree: number, isMorning: boolean, env: Env): Promise<string> {
  ...
  const prompt = `Ты — мудрый и вдохновляющий ассистент. Пользователь бросил курить ровно ${daysFree} дней назад.
Напиши ОДНО короткое, вдохновляющее и глубокое предложение на русском языке для ${timeOfDay}, чтобы поддержать его выбор ясности, свободы и здоровья.
Требования к ответу:
1. Максимально короткое и емкое (одно предложение).
2. На русском языке.
3. Без банальностей, штампов или пустых лозунгов (избегай фраз типа "ты справишься", "каждый день — это победа", "верь в себя", "просто сделай это").
4. Сфокусируйся на ценности осознанности, глубокого дыхания, чистого разума и личной силы.
5. Не используй кавычки вокруг предложения и не нумеруй его.`;
  ...
  const payload = {
    ...
    messages: [
      {
        role: "system",
        content: "Ты — вдохновляющий ментор, пишущий емкие и оригинальные афоризмы без клише."
      },
      {
        role: "user",
        content: prompt
  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === "/preview/morning" || path === "/preview/evening") {
      ...
      if (env.DEEPSEEK_API_KEY) {
        ...
        advice = isMorning 
          ? "Чистый воздух в легких — это лучшее начало дня, свободного от иллюзий." 
          : "Каждый шаг в сторону чистоты ума приближает тебя к истинному себе.";
      }
      const message = isMorning ? getMorningMessage(daysFree, advice) : getEveningMessage(daysFree, advice);
